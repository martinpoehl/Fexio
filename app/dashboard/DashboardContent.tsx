...
function StatCard({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="flex-1 min-w-[160px] bg-white rounded-lg border border-gray-200 p-5">
  ...
  return (
    <div>
      ...
      <div className="flex gap-3.5 flex-wrap mb-6">
        <StatCard label="Umsatz (bezahlt)" value={fCHF(stats.revenue)} color="text-green-700" />
        <StatCard label="Offene Rechnungen" value={fCHF(stats.outstanding)} color="text-amber-600" />
        <StatCard label="Aufwendungen" value={fCHF(stats.expenses)} color="text-red-600" />
        <StatCard label="Stunden (Woche)" value={fDur(stats.weekMinutes)} color="text-blue-600" />
      </div>
